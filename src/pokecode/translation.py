...
class TranslationService:
    ...
    def translate_country_name(self, country_name: str) -> str:
        ...
        # APIで翻訳
        try:
            result = self._call_openai_api([country_name])
            if result and country_name in result:
    ...
    def translate_countries_batch(
        ...
        *,
        ...
        Args:
            countries: 国情報のリスト（country_nameを含む）
        ...
        # キャッシュにない国を特定
        to_translate = [
            name for name in country_names if name not in self.cache
        ]
    ...
    def _translate_batch(
        ...
        Args:
            country_names: 翻訳対象の国名リスト
            batch_size: 1回のAPI呼び出しで処理する数
        """
        for i in range(0, len(country_names), batch_size):
            batch = country_names[i : i + batch_size]
            logger.info(
                f"バッチ処理: {i + 1}-{min(i + batch_size, len(country_names))}"
            )
    ...
    def _call_openai_api(
        ...
        try:
            ...
            if len(translations) != len(country_names):
                logger.warning(
                    f"翻訳数が一致しません: {len(translations)} != {len(country_names)}"
                )
                return None
