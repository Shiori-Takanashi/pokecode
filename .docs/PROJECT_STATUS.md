# プロジェクトステータス（2026-02-05）

## 概要

**pokecode** は HTTP リクエスト（HTML/JSON）を適切にハンドルするための Python ライブラリです。設定管理、ロギング、エラーハンドリングが統合されています。

---

## 現在のフェーズ

🎯 **PHASE07 完了** - 設定管理をシンプルな関数型に統一

```
PHASE01-05: ✅ 実装完了（request_html, request_json）
PHASE06:    ✅ 実装完了（設定管理リファクタリング）
PHASE07:    ✅ 実装完了（関数型 API 統一）
PHASE08:    🔴 次のフェーズ（テスト基盤）
```
...
| `request_json()` | JSON 取得（形式検証付き） | [request_json.py](../src/pokecode/request_json.py) |
...
# JSON 取得
data = request_json("https://api.example.com/data")
```
...
host = config.get_host()  # 127.0.0.1
log_dir = config.get_log_dir()  # logs
